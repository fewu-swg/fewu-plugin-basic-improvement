...
function txt(ctx: BasicContext) {
    ...
    for (const v of pages) { result += encodeURI(ctx.config.url + v.url) + '\n' }
    return result;
}
...
function xml(ctx: BasicContext) {
    ...
    result += `<url><loc>${encodeURI(ctx.config.url)}</loc><priority>1.0</priority></url>`;
    for (const v of pages) {
        result += `<url><loc>${encodeURI(ctx.config.url + v.url)}</loc><lastmod>${v.time.getFullYear()}-${v.time.getMonth() + 1}-${v.time.getDate()}</lastmod></url>`;
    }
    result += '</urlset>';
    return result;
}
...
export default function installSitemap(_ctx: BasicContext) {
    _ctx.on('ready', async (ctx: BasicContext) => {
        ...
        if (sitemap_type === 'txt') {
            ...
            await writeFile(sitemap_path, xml(ctx));
        }
    });
}